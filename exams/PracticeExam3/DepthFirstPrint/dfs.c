/* TO COMPILE: gcc -Wall -ansi -o program dfs.c */
...
void DepthFirstPrint(struct BSTree *tree){
    ...
    /* Step 1: initialize the stack, add the root to the stack, set the flag*/
    s = (struct listStack *) malloc (sizeof(struct listStack));
    initListStack(s);
    pushListStack(s, current);
    current->processed = 1;

    /* Step 2: We iterate through the stack */
    while (!isEmptyListStack(s)) {
        current = topListStack(s);
        /* Step 3a: if the top element is not yet processed, then add it to the stack */
        if ((current->left != 0 && !current->left->processed) || (current->right != 0 && !current->right->processed)) {
            if (current->right != 0) {
                pushListStack(s, current->right);
            }
            
            if (current->left != 0) {
                pushListStack(s, current->left);
            }
        }

        /* Step 3b: otherwise, process the node, indicate it's been processed and remove it */
        else {
