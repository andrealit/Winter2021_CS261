...
{ 
   ...
   /* Since it's a queue, you need to add to the back */
   addBackDList(q, val);
...
void  _addDLink (struct DList *dq, struct DLink *lnk, TYPE val) 
{
   ...
   /* FIX ME */
   /* assert(lnk!=dq->frontSentinel); */

   /* 1. allocate memory for the new link, assign a value */
   ...
   /* Step 3: properly attach the deque to the new link */
...
{  
   ...
   /* allocate new memory for a DList */
   struct DList * newqueue = (struct DList *) malloc (sizeof(struct DList));
   assert(newqueue);

   /* IMPORTANT: Remember to initialize the new queue */
   initDList(newqueue);

   /* copy over elements from back to front for reverse */
   /* struct DLink * current = q->backSentinel->prev; */
   /* while (current != q->frontSentinel) { */
       /* addQueue(newqueue, current->value); */
       /* current = current->prev; */
   /*}*/

   struct DLink * current = q->frontSentinel->next;
   while (current != q->backSentinel) {
       addFrontDList(newqueue, current->value);
       current = current->next;
   }
...
/* Remove an element from the list if it is equal */
void removeListBag(struct ListBag *b, TYPE val) {
   struct Link * previous = b->sentinel;
   struct Link * current = b->sentinel->next;
   while (current != NULL) { /* we iterate until current hits null */
      /* the value is equal */
      if (EQ(current->value, val)) {
         previous->next = current->next; /* disconnect current */
         free(current); /* free the current */
         return;
      }
      /* else, iterate through to the next element */
      previous = current;
      current = current->next;
   }
}

/* Remove All Occurrances of the element */ 
void removeAllOccurances(struct ListBag *b, TYPE val) {
   struct Link *previous = b->sentinel;
   struct Link *current = b->sentinel->next;
   while (current != NULL) {
      if (EQ(current->value, val)) {
         ...
         free(current);
         current = previous; /* only modification is getting rid of return */
      }
      previous = current;
      current = current->next;
   }
...
/* Remove that from the deque */
void removeDList(struct dlist * ls, struct dlist *lnk) {
   assert(ls && !isEmptyDeque(ls));

   /* assign the previous link's next and skip over the next element */
   lnk->prev->next = lnk->next;
   /* assign the next link's previous and assign it to the link before prev */
   lnk->next->prev = lnk->prev;

   /* 3. remove from memory */
   free(lnk);

   /* 4. get the size minimized */
   ls->size--;
}
