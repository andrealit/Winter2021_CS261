...
int main (int argc, const char * argv[]) {
	...
	while (!feof(fileptr)) {
		...
		/* at map searches the tree and returns the value of the word */
		ptrToValue = atMap(&hashTable, eachWord);

		/* if the ptr to the slot is empty, add the word into the map */
		if (containsKey(&hashTable, eachWord) == 0) {
