...
struct AVLnode * rotateLeft(struct AVLnode * current)
{
    ...
    /* switch the right with the left node of the right */
    current->right = newtop->left;
    newtop->left = current;
   
    /* set the height of the nodes by correcting it */
    setHeight(current);
	setHeight(newtop);
...
struct AVLnode * rotateRight(struct AVLnode * current)
{
    ...
    /* switch the left with the right node of the left */
...
struct AVLnode * _balance(struct AVLnode * current)
{
    ...
    if (cbf < -1) {
		...
		/* handle the double rotation case! */
        ...
        if (bf(current->right) < 0) {
            /* using assigning the right to the new top to rotate once */
...
struct AVLnode * AVLnodeAdd(struct	AVLnode * current, TYPE newValue)
{
    ...
    /* FIX ME */
	if (current == 0) {
        struct AVLnode * addedNode = (struct AVLnode *)malloc(sizeof(struct AVLnode));
        assert(addedNode);
        ...
        /* recursive call to the left if less than, to find the lesser node */
        if (LT(newValue, current->val)) {
            current->left = AVLnodeAdd(current->left, newValue);
        } else {
            /* case where it's both equal and greater than */
            current->right = AVLnodeAdd(current->right, newValue);
        }

        /* balance the tree to ensure AVL tree stays in correct order after you finish adding */
