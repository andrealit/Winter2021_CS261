...
void addHeap(DynArr *heap, TYPE node)
{
    ...
    int pidx = (pos - 1)/2;     /* get parent value */
    ...
    /* percolate up */
    while (pos > 0 && LT(node.priority, parentVal.priority)) {
        heap->data[pos] = parentVal;
        pos = pidx; /* new position to add */
...
/* Helper function to double the heap's capacity */
void _doubleCapacity(DynArr *heap, int cap) {
    heap->capacity = cap;
}
...
/* Build heap
	...
	Given an array of data, construct the heap
	At each step, the subtree rooted at i becomes a heap
	Loop toward the root until the entire dynamic array becomes a heap
	 
*/
void buildHeap(DynArr * da) {
	int maxIdx = da->size;
	int i;

	/* at the beginning, only leaves are proper heaps */
...
/* 
	...
	1. Build heap from unsorted data
	2. For (i = data->size-1; i < 0; i--)
		a. swap the root with the last element
		b. adjust the heap from index 0 to i

*/
void heapSort (DynArr * data) {
	int i;
	buildHeap(data);
	for (i = data->size - 1; i > 0; i--) {
		swapDynArr(data, 0, i); /* swap i-th element with root */
		_adjustHeap(data, 0, i); /* percolate root down */
...
*/
void addHeapRecurse(DynArr * heap, TYPE e) {
	assert(heap);

	if (heap->size >= heap->cap) {
		_doubleCapacity(heap, 2 * heap->cap);
	}
	/* auxiliary recursive function: begin adding e to last position */
...
void _heapAdd(DynArr * heap, TYPE e, int index) {
	int pidx = (index - 1)/2; /* parent index */

	/* Do we need to percolate up? */
	if (index > 0 && LT(e, heap->data[pidx])) {
		/* the heap property is violated */
		heap->data[index] = heap->data[pidx]; /* swap */
		_heapAdd(heap, e, pidx); /* recursive */

	} else {
		/* stop recursion */

		/* Now add the new element to the heap */
		heap->data[index] = e;
	}
}

/* Input: heap -- pointer to the heap
		  da -- pointer to the dynamic array of elements to be removed
*/
void rmHeap (DynArr * heap, DynArr * da) {
	int i, j;
	for (i = 0; i < da->size; i++) { /* loop over da */
		j = 0;
		while (j < heap->size) { /* loop over heap */
...
/* Remove a certain node from a heap */



/* contains heap */
int containsHeap(DynArr *heap, TYPE e) {
	assert(heap);
	/* FIX ME */

	return _findIndexHeap(heap, e, 0);
